...
class App extends Component {
  ...
  clickThumbnailItem = imgUrl => {
    const {displayImgUrl, count} = this.state
    if (displayImgUrl === imgUrl && count > 0) {
      const randomImgUrl =
        imagesList[Math.ceil(Math.random() * imagesList.length - 1)].imageUrl
      this.setState(prevState => ({
        displayImgUrl: randomImgUrl,
  ...
  render() {
    const {score, count, displayImgUrl, activeTabId, showScoreCard} = this.state
        ...
        <div className="header-background">
          ...
          <ul className="score-and-timer-container">
            <li className="category-container">
              <p className="category-text">Score:</p>
              <p className="category-value">{score}</p>
            </li>
            <li className="category-container">
              <img
              ...
              />
              <p className="category-value">{`${count} Sec`}</p>
            </li>
          </ul>
        </div>
        {!showScoreCard && (
          <div className="game-display-background-container">
            <div className="game-display-responsive-container">
              <img src={displayImgUrl} alt="match" className="display-image" />
              ...
              <div className="thumbnail-items-container">
                <ul className="thumbnail-items-list">
                  {imagesList
                    .filter(eachImage => eachImage.category === activeTabId)
                    .map(eachThumbnail => (
                      <ThumbnailItem
